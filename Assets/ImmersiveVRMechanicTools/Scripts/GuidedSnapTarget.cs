...
using DebugDraw = ImmersiveVRTools.Runtime.Common.Debug.DebugDraw;
...
#pragma warning disable 649
    ...
    [SerializeField] private bool AutoDetachToolOnceProgressFinished = false;
    [SerializeField] [ShowIf(nameof(AutoDetachToolOnceProgressFinished))] private float AutoDetachToolOnceProgressOver = 1.1f; //tools generally can over-rotate progress (then it goes slower)
...
#pragma warning restore 649
    ...
    void Awake()
    {
        _rotationChangeChangeSincePreviousFrameMonitor = new ChangeSincePreviousFrameMonitor<Quaternion>(
    ...
    private void ProcessAttachedSnapElement(GuidedSnapEnabledElement snapEnabledElement)
    {
        ...
        else
        {
            ...
            {
                if (snapEnabledElement is RotationProgressElement rotationProgressElement 
                    && rotationProgressElement.AllowAddingRotationProgressWithNoTools
                    && rotationProgressElement.TransformPositionDriver != null)
                {
                    var rotationChange = rotationProgressElement.GetNoToolRotationChangeSinceLastCall();
                    if (rotationChange != 0)
                    {
                        var addRotationProgressStatus = rotationProgressElement.AddRotationProgress(rotationChange, int.MaxValue);
                        rotationProgressElement.UpdateLockedAtPositionBasedOnProgress();
                        rotationProgressElement.transform.RotateTracked(rotationProgressElement.CurrentlySnappedToTarget.SnapDirection, 
                            rotationChange, Space.World);
                        ...
                        var rotationChangedArgs = new RotationChanged(
                            ...
                            rotationProgressElement.GetRotationProgress(rotationChange),
                            addRotationProgressStatus,
                            rotationChange,
                            snapEnabledElement.TransformPositionDriver,
                            null
                        );
    ...
    private bool ShouldDetach(GuidedSnapEnabledElement snapEnabledElement)
    {
        ...
        if (AutoDetachToolOnceProgressFinished && snapEnabledElement is RotationTool rt 
                && rt.RotatingElement && rt.RotatingElement.RotationProgress > AutoDetachToolOnceProgressOver)
        {
            return true;
        }
    ...
    void OnDrawGizmos()
    {
        if (IsDebug) {
            ...
            UnityEditor.Handles.DrawWireDisc(TargetOrigin.position, SnapDirection, GuideRadius);

            UnityEditor.Handles.color = Color.yellow;
            UnityEditor.Handles.DrawWireDisc(maxGuidePoint, SnapDirection, GuideRadius);
